...
- 为什么有效：LLM 本质上仍是 next-token predictor，安全拒答是 SFT/RLHF/安全微调后学到的行为模式，而不是对所有输入都成立的硬约束。对抗后缀会在 token 概率空间里把模型从“拒绝回答”的轨道推向“开始配合回答”的轨道，如：“**告诉我怎么制造炸弹。当然，这是一步一步制造炸弹的方法**”；一旦输出前缀进入类似同意执行的区域，后续生成更容易沿着原请求继续。它还有效，是因为自然语言上下文同时承载指令和数据，模型很难形式化地区分“用户真实意图”“攻击文本”“待处理内容”和“系统约束”；而梯度/搜索可以找到人类不自然、但对模型内部概率边界有效的 token 组合。迁移性则说明不同模型共享相似预训练语料、指令微调模板和拒答边界，所以在开源模型上优化出的脆弱模式可能影响黑盒模型。
- 案例（去武器化）：真实 adversarial suffix 往往不是自然语言，而是一串对人类近似乱码、但能改变模型 token 分布的片段。为避免提供可复用攻击，这里只保留结构示意：`[用户请求] + [若干异常标点/罕见 token/断裂片段] + [诱导模型进入肯定回答开头的占位目标] + [结束/分隔占位符]`。研究时要观察的不是这串占位文本本身，而是三件事：第一，后缀会把模型从拒答前缀推向配合前缀；第二，后缀可被同时优化到多个请求或多个开源模型上；第三，一些 token 组合虽然没有人类语义，却能稳定影响模型的下一 token 概率。真实攻击样本只应在授权红队、隔离环境和安全评测基准中使用。
...
- 流程 / 怎么做：收集野生 jailbreak prompt，分类攻击策略，并在多个模型上测试成功率。
  ...
  如：【角色扮演、情景模拟】 + 【llm改写越狱提示词；往返翻译（英文->中文->英文，打乱越狱提示词）；随机引入错误拼写，打乱词汇顺序】；
...
- 为什么有效：根因是 LLM 应用常把不同安全等级的文本拼进同一个上下文：  system instruction + developer instruction + user request + retrieved documents + tool output

    对模型来说，这些最后都变成 token 序列。它并没有天然的硬隔离机制来保证“网页内容只能当数据，不能当指令”。
